...
export default function BuyerStats() {
  ...
  // State dữ liệu
  const [stats, setStats] = useState<BuyerStatsData>({
    totalSpent: 0,
    totalOrders: 0,
    completedOrders: 0,
    cancelledOrders: 0,
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const data = await OrderService.getBuyerStats();

        // Map dữ liệu từ Backend vào State của Recharts
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">Thống kê mua</h1>
